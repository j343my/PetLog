...
export default function PetForm({ config, onChange, onPrint }: PetFormProps) {
    ...
    return (
        <div className="no-print bg-slate-800 text-white shadow-md mb-8 sticky top-0 z-50">
            {/* Collapsed header bar - always visible */}
            <div
                className="flex items-center justify-between px-6 py-3 cursor-pointer select-none hover:bg-slate-700 transition-colors"
                onClick={() => setIsOpen(!isOpen)}
            >
                <div className="flex items-center gap-3">
                    <h1 className="text-lg font-bold">PetLog</h1>
                    <span className="text-slate-400 text-sm">— {config.petName} · {config.targetMonth}</span>
                </div>
                <div className="flex items-center gap-3">
                    <button
                        onClick={(e) => { e.stopPropagation(); onPrint(); }}
                        className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-4 py-1.5 rounded-md font-bold transition-colors text-sm"
                    >
